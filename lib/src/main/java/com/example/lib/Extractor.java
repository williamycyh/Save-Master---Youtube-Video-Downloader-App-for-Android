...
public abstract class Extractor extends AsyncTask<String, Void, SparseArray<YFile>> {
    ...
    private static final SparseArray<MyInsAc.Format> FORMAT_MAP = new SparseArray<>();

    private String decodeStr(String encodeStr){
        return BaseCommon.decodeToString(encodeStr);
    }

    public MyInsAc.Format getFormat(String encode){

        String[] list = encode.split(",");
        MyInsAc.Format format = null;
        try{
            format = new MyInsAc.Format(Integer.valueOf(list[0].trim()),list[1].trim(),Integer.valueOf(list[2].trim()), Integer.valueOf(list[3].trim()),Integer.valueOf(list[4].trim()));
    ...
    public void initFormat(String encodeStr){
        MyInsAc.Format format = getFormat(BaseCommon.decodeToString(encodeStr));
        if(format != null){
